...
const updateOptionsBlock = (source, label, items) => {
  const pattern = new RegExp(
    `(label:\\s*\\"${label}\\"[\\s\\S]*?options:\\n)([\\s\\S]*?)(?=\\n\\s*(?:- label:|- \\{ label:|$))`,
    "m"
  );
  const replacement = `$1${list(items, 10)}`;
...
let yml = updateOptionsBlock(existingYml, "Structural", structuralSections);
yml = updateOptionsBlock(yml, "Topics", topicTags);
